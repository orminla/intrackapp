...
<html lang="en">
    <head>
        ...
        <link
            rel="shortcut icon"
            href="{{ asset("$assetPath/images/logos/favicon.png") }}"
        />
        <link
            rel="stylesheet"
            href="{{ asset("$assetPath/css/styles.min.css") }}"
        />
    ...
    <body>
            ...
            >
                ...
                >
                    ...
                    <div class="col-md-6 d-none d-md-block bg-white p-3">
                        <img
                            src="{{ asset("$assetPath/images/sign_in.jpg") }}"
                            alt="Welcome Image"
                    ...
                    >
                        <div class="text-center mb-4 mt-4">
                            <img
                                src="{{ asset("$assetPath/images/logo_intrackapp.png") }}"
                                alt="InTrack App"
                                style="max-width: 70%"
                            />
                        </div>
        ...
        {{-- Include Modals --}}
        @include("auth.forget_modal")
        @include("auth.reset_modal")

        <script src="{{ asset("$assetPath/libs/jquery/dist/jquery.min.js") }}"></script>
        <script src="{{ asset("$assetPath/libs/bootstrap/dist/js/bootstrap.bundle.min.js") }}"></script>
        ...
        <script>
            // Toggle password
                ...
                document.getElementById("loginForm").addEventListener("submit", async function(e) {
                    ...
                    try {
                        let res = await fetch("{{ route('login.post') }}", {
                            method: "POST",
                            headers: {
                        ...
                        if(res.ok) {
                            ...
                            setTimeout(() => {
                                if(data.user.role === 'admin') window.location.href = "{{ route('admin.dashboard') }}";
                                else window.location.href = "{{ route('inspector.dashboard') }}";
                            }, 1500);
                ...
                @if(session('verifikasi_status') && session('verifikasi_message'))
                    Swal.fire({
                        icon: "{{ session('verifikasi_status') }}",
                        title: "{{ session('verifikasi_status') === 'success' ? 'Berhasil!' : 'Gagal!' }}",
                        text: "{{ session('verifikasi_message') }}",
