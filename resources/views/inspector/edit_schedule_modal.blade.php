...
@foreach ($reports as $i => $schedule)
    <div
        class="modal fade"
        id="editScheduleModal-jadwal-{{ $i }}"
        tabindex="-1"
        aria-labelledby="editScheduleModalLabel-jadwal-{{ $i }}"
        aria-hidden="true"
    >
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content rounded-4 p-3 modal-lg-scrollable">
                <form
                    method="POST"
                    action="{{ route("inspector.jadwal.update", $schedule["id"]) }}"
                    enctype="multipart/form-data"
                    class="editable-form"
                    data-form-index="jadwal-{{ $i }}"
                >
                    @csrf
                    @method("PUT")
