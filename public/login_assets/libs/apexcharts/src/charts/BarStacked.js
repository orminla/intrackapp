...
class BarStacked extends Bar {
    ...
    drawStackedColumnPaths({
        ...
        for (let k = 0; k < this.groupCtx.prevYF.length; k++) {
            ...
            prevBarH =
                prevBarH + (!isNaN(this.groupCtx.prevYF[k][j]) ? this.groupCtx.prevYF[k][j] : 0);
        }
        ...
        if (
            ...
            if (
                ...
                this.groupCtx.prevYF.slice(1, gsi).every((arr) => arr.every((val) => isNaN(val)))
            ) {
                barYPosition = zeroH;
            } else {
                // Nothing special
        ...
        if (this.series[i][j]) {
            y =
                ...
                (this.isReversed ? this.series[i][j] / this.yRatio[translationsIndex] : 0) * 2;
        } else {
            // fixes #3610
            y = barYPosition;
        }
