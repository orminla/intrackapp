...
class Line {
    ...
    draw(series, ctype, seriesIndex, seriesRangeEnd) {
        ...
        if (typeof w.config.series[0]?.zIndex !== 'undefined') {
            allSeries.sort(
                (a, b) =>
                    Number(a.node.getAttribute('zIndex')) - Number(b.node.getAttribute('zIndex')),
            );
        }
    ...
    _iterateOverDataPoints({
        ...
        for (let j = 0; j < iterations; j++) {
            ...
            if (
                isNull &&
                (w.config.stroke.curve === 'smooth' || w.config.stroke.curve === 'monotoneCubic')
            ) {
                yArrj.push(null);
                y2Arrj.push(null);
            } else {
            ...
            case 'monotoneCubic':
                ...
                switch (pathState) {
                    ...
                    case 2:
                        ...
                        const points =
                            smoothInputs.length > 1 ? spline.points(smoothInputs) : smoothInputs;
