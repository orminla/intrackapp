 ...
 */ (function (root, factory) {
    ...
    SVG.adopt = function (node) {
        ...
        if (node.nodeName == 'svg') {
            element =
                node.parentNode instanceof window.SVGElement ? new SVG.Nested() : new SVG.Doc();
    ...
    SVG.prepare = function () {
        ...
        var body = document.getElementsByTagName('body')[0],
            draw = (body ? new SVG.Doc(body) : SVG.adopt(document.documentElement).nested()).size(
                2,
                0,
            );
        ...
        at: function (pos) {
            ...
            for (var i = 0, il = sourceArray.length; i < il; i++) {
                ...
                for (var j = 1, jl = sourceArray[i].length; j < jl; j++) {
                    array[i][j] =
                        sourceArray[i][j] + (destinationArray[i][j] - sourceArray[i][j]) * pos;
                }
                ...
                if (this.situation.loops !== false) {
                    ...
                    if (this.situation.reversing) {
                        ...
                        this.situation.reversed =
                            this.situation.reversed !=
                            Boolean((this.situation.loop - lastLoop) % 2);
                    }
                } else {
            ...
            eachAt: function () {
                ...
                for (var i in s.animations) {
                    at = [].concat(s.animations[i]).map(function (el) {
                        return typeof el !== 'string' && el.at
                            ? el.at(s.ease(self.pos), self.pos)
                            : el;
                    });
        ...
        fire: function (event, data) {
            ...
            if (event instanceof window.Event) {
                ...
                this.node.dispatchEvent(
                    (event = new SVG.CustomEvent(event, { detail: data, cancelable: true })),
                );
            }
    ...
    SVG.Doc = SVG.invent({
        ...
        extend: {
            // Add namespaces
            namespace: function () {
                return this.attr({ xmlns: SVG.ns, version: '1.1' })
            ...
            parent: function () {
                if (!this.node.parentNode || this.node.parentNode.nodeName == '#document')
                    return null;
    ...
    SVG.extend(SVG.Circle, SVG.Ellipse, {
        ...
        size: function (width, height) {
            ...
            return this.rx(new SVG.Number(p.width).divide(2)).ry(
                new SVG.Number(p.height).divide(2),
            );
        },
    });
    SVG.Line = SVG.invent({
    ...
    SVG.Text = SVG.invent({
        ...
        extend: {
            ...
            lines: function () {
                ...
                var lines = SVG.utils.map(
                    SVG.utils.filterSVGElements(node.childNodes),
                    function (el) {
                        return SVG.adopt(el);
                    },
                );
