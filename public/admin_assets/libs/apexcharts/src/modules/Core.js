...
export default class Core {
    ...
    setupElements() {
        ...
        gl.isBarHorizontal =
            ['bar', 'rangeBar', 'boxPlot'].includes(ct) && cnf.plotOptions.bar.horizontal;
    ...
    plotChartType(ser, xyRatios) {
        ...
        gl.series.forEach((serie, st) => {
            ...
            if (seriesTypes[seriesType]) {
                ...
                ['heatmap', 'treemap', 'pie', 'donut', 'polarArea', 'radialBar', 'radar'].includes(
                    seriesType,
                )
            ) {
        ...
        if (comboCount > 0) {
            ...
            if (seriesTypes.column.series.length > 0 && cnf.plotOptions.bar.horizontal) {
        ...
        if (gl.comboCharts) {
            ...
            if (seriesTypes.area.series.length > 0) {
                elGraph.push(
                    ...coreUtils.drawSeriesByGroup(seriesTypes.area, gl.areaGroups, 'area', line),
                );
            }
            if (seriesTypes.column.series.length > 0) {
                if (cnf.chart.stacked) {
                    const barStacked = new BarStacked(ctx, xyRatios);
                    elGraph.push(barStacked.draw(seriesTypes.column.series, seriesTypes.column.i));
                } else {
                    ctx.bar = new Bar(ctx, xyRatios);
                    elGraph.push(ctx.bar.draw(seriesTypes.column.series, seriesTypes.column.i));
                }
            }
            ...
            if (seriesTypes.line.series.length > 0) {
                elGraph.push(
                    ...coreUtils.drawSeriesByGroup(seriesTypes.line, gl.lineGroups, 'line', line),
                );
            }
            ...
            if (seriesTypes.scatter.series.length > 0) {
                ...
                elGraph.push(
                    scatterLine.draw(seriesTypes.scatter.series, 'scatter', seriesTypes.scatter.i),
                );
            }
            if (seriesTypes.bubble.series.length > 0) {
                const bubbleLine = new Line(ctx, xyRatios, true);
                elGraph.push(
                    bubbleLine.draw(seriesTypes.bubble.series, 'bubble', seriesTypes.bubble.i),
                );
            }
        } else {
