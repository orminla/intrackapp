...
export default class DimXAxis {
    ...
    getxAxisGroupLabelsCoords() {
        ...
        const fontSize =
            w.config.xaxis.group.style?.fontSize || w.config.xaxis.labels.style.fontSize;
        ...
        const rightPad = (yaxe) => {
            if (this.dCtx.timescaleLabels && this.dCtx.timescaleLabels.length) {
                ...
                const lastLabelPosition =
                    lastTimescaleLabel.position + lbWidth / 1.75 - this.dCtx.yAxisWidthRight;

                const firstLabelPosition =
                    firstimescaleLabel.position - lbWidth / 1.75 + this.dCtx.yAxisWidthLeft;
